...
var List = Vue.extend({
	...
	mounted: function () {
		...
		$( ".bs-gallery-photos-status-modal-lg" ).on('shown.bs.modal', function(event){
			...
			MV.api.readList('/emvarias_reports_photographic', {
				filter: [
	...
	methods: {
					...
					self.createLogSchedule({
						...
						bootbox.confirm({
							...
							callback: function (result) {							if(result === true){
									urlWA = 'https://wa.me/57' + file.create_by.mobile + '?text=Se%20ha%20rechazado%20una%20FOTO' + '%20M:%20' + file.schedule.lot.microroute_name + '.%0AIngresa%20a%20https%3A%2F%2Fmicuenta.monteverdeltda.com%20para%20gestionarla.';
		...
		jsonEvent(a){
			...
			var totalProg = 0;
			var photosReq = (parseInt(parseFloat(a.lot.area_m2) * self.baseAreaCalc_m2_photos) < self.minPhotosSpaceSmall) ? self.minPhotosSpaceSmall : parseInt(parseFloat(a.lot.area_m2) * self.baseAreaCalc_m2_photos);
			
			a.emvarias_reports_photographic.forEach(function(b){
				//console.log(b);
			...
			ret.push(a.id);
			ret.push("<a>" + a.lot.microroute_name + '</a>');
			ret.push(a.lot.id_ref);
		...
		load(){
			...
			api.get('/records/emvarias_schedule', { params: {
				join: [
					'emvarias_lots',
					'emvarias_lots,emvarias_groups',
					'emvarias_lots,emvarias_periods',
					'emvarias_groups',
					'emvarias_periods',
