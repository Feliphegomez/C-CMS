...
spl_autoload_register(function($className){
		...
		$ruta_1 = dirname(CONFIG_PATH . '../') . "/model/" . (empty($namespace) ? "" : $namespace . "/") . "{$className}.php";
	...
	try {
		if(@get_called_class() != false){			
			$ruta_4 = dirname(CONFIG_PATH . '../') . "/model/" . str_replace(substr(get_called_class(), 0, strrpos(get_called_class(), "\\")) . "/","", $className) . ".php";
